...
**示例：**
1. "用 Crazyrouter 一个 API Key 接入 469 个模型，比逐个配置官方 API 省下大量接入时间"
2. "Crazyrouter 让 AI 开发者用统一 API 访问 30+ 供应商模型，价格比官方低 30%-50%"
3. "从 OpenRouter 迁移到 Crazyrouter，5 分钟完成，并保留多协议接入能力"
...
```yaml
thesis:
  final: "用 Crazyrouter 一个 API Key 接入 469 个模型，比逐个配置官方 API 省下大量接入时间"
```
