...
- ✅ **Topic 去重层** - 基于 `Topics/` 与本地索引识别新建、更新、重复风险
- ✅ **Vault 索引重建** - 支持对 `D:/crazyrouter` 这类 Obsidian vault 做结构初始化和重建
...
seomaster publish draft.md        # 两步发布：先创建草稿，再调用 publish 接口
...
seomaster publish output/my-article-draft.md --project crazyrouter
```
...
```bash
seomaster publish output/my-article-draft.md --project crazyrouter
seomaster publish my-article-draft.md --project crazyrouter
```

- `publish` 默认走 `new-api` 两步流程：先 `POST /api/blog/admin/` 创建草稿，再 `POST /api/blog/admin/:id/publish`
- 默认会在发布后执行一次 `sync:blog incremental` 回填本地镜像、`Published/` 和索引
- 可通过 `NEW_API_BLOG_TOKEN` / `NEW_API_BLOG_USER_ID` 或兼容的 `ADMIN_TOKEN` / `ADMIN_USER_ID` 提供管理端凭证
